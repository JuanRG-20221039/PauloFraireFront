...
const UserProfile = () => {
    ...
    const handleSelectOffer = async (offerId) => {
        ...
        try {
            ...
            Swal.fire("Error", error.response?.data?.message || "No se pudo seleccionar la oferta educativa", "error");
        } finally {
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        {userData.selectedEducationalOffer ? (
                            <div className="mb-4">
                                <p className="text-gray-600 font-medium">Oferta Educativa Seleccionada:</p>
                                <div className="flex items-center justify-between mt-1 p-2 border rounded bg-blue-50">
                                    <p className="text-gray-800 font-semibold">
                                        {educationalOffers.find(offer => offer._id === userData.selectedEducationalOffer)?.title || 'Cargando...'}
                                    </p>
                                    <button
                            ...
                            <div className="mb-4">
                                <p className="text-gray-600 font-medium mb-2">Seleccione una Oferta Educativa:</p>
                                {loadingOffers ? (
                                    <p className="text-gray-600 italic">Cargando ofertas educativas...</p>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        >
                                            ...
                                            {educationalOffers.map(offer => (
                                                ...
                                                >
                                                    {offer.title} - Cupos: {offer.availableSpots}/{offer.maxCapacity} {offer.isFull ? '(Sin cupo)' : ''}
                                                </option>
                                            ))}
                                        </select>
                                    </div>
                                ) : (
                                    <p className="text-gray-600 italic">No hay ofertas educativas disponibles.</p>
                                )}
                            </div>
                        )}
                    </div>
                            ...
                            {docs.length > 0 && (
                                <>
                                    ...
                                    <button
                                        ...
                                        disabled={isUploading || !userData.selectedEducationalOffer}
                                    ...
                                    >
                                        {isUploading ? `Subiendo... ${uploadProgress}%` : userData.selectedEducationalOffer ? "Subir Documentos" : "Seleccione una oferta educativa primero"}
                                    </button>
                                </>
                            )}
                        </div>
